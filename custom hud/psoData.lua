...
local psoData = {}
local offset = {
	bossHP = {[45]=0x6b4, [73]=0x704},
	bossMaxHP = {[45]=0x6b0, [73]=0x700},
	evp = 0x2d0,
	HP = 0x334,
	item ={weapon=0x00,armor=0x04,mag=0x10,tool=0x0c,unit=0x08},
	killCount = 0xe8,
	maskHP = {[45]=0x6b8, [73]=0x708},
	monsterUnitxtID = 0x378,
	myPlayerIndex = 0x00a9c4f4,
	playerArray = 0x00a94254,
	result={weapon=44,armor=32,mag=28,tool=24,unit=20},
	room = 0x28,
	shellHP = {[45]=0x39c, [73]=0x7ac},
	toolQuantity = 0x104,
	weaponGrind = 0x1f5,
} -- local offset = {...}
local types = {
	armor = {"frame", "barrier", "unit"},
	item = {"weapon", "armor", "mag", "tool", "meseta"},
	attribute = {"native", "a. beast", "machine", "dark", "hit"},
	tech = {[9]="s", [10]="d", [11]="j", [12]="z"},
} -- local types = {...}
local session = {
	startXP,
...
local function getself()
	return pso.read_u32(offset.playerArray + pso.read_u32(offset.myPlayerIndex) * 4)
...
local function initsessiontime()
	...
	if player ~= 0 then
		session.startXP = pso.read_u32(player + 0xe48)
	end
...
local function getunitxtitemaddress(type, group, index)
	...
	-- print(type)
	local itemaddr = pso.read_u32(pmtaddress + offset.item[type])
	-- print(itemaddr)
	...
	if index < count and itemaddr ~= 0 then
		return itemaddr + (index * offset.result[type])
	else
		return nil
	end
...
local function readitemdata(itemaddr, location)
	...
	item.type = types.item[pso.read_u8(itemaddr + offset) + 1]
	...
	if item.type == "weapon" then
		...
		if location == "bank" then
			...
			grindoffset = offset.weaponGrind
			specialoffset = 0x1f6
			statsoffset = 0x1c8
		end
		
		item.grind = pso.read_u8(itemaddr + offset.weaponGrind)
		...
		if group == 0x33 or group == 0xab then
			item.killcount = pso.read_u16(itemaddr + offset.killCount)
		end
		...
		if item.issrank then
			...
			for attrname in ipairs(types.attribute) do
				item[attrname] = 0
			end
			for i = 0,4,2 do
				...
				if attrindex < 6 then -- maybe this 6 should be a 7
					...
					local attrname = types.attribute[attrindex]
	...
	elseif item.type == "tool" then
		if group == 2 then -- technique disk: tech learned, level
			...
			if location == "bank" then
				...
				item.quantity = bit.bxor(pso.read_u32(itemaddr + offset.toolQuantity), itemaddr + offset.toolQuantity)
			end
		end
	elseif item.type == "meseta" then
		-- meseta: quantity
...
local function readbuffdata(playeraddr, offset)
	...
	if techtype == 0 then
		...
		techdata.name = types.tech[techtype]
...
local function readplayermonsterdata(playeraddr)
	local player = {}
	player.hp = pso.read_u16(playeraddr + offset.HP)
...
function psoData.retrievepsodata()
	...
	local frametime = os.time() - session.lastFrameTime
	...
	if playeraddr ~= 0 then
		...
		if activegamedata.party then
			gamedata.party = {}
			for i = 0, 11 do
				local partymemberaddr = pso.read_u32(offset.playerArray + i * 4)
		...
		if activegamedata.sessiontime then
			...
			if location ~= _lobby then
				if location == session.lastLocation then
					gamedata.elapsedtime = gamedata.elapsedtime + frametime
					gamedata.sessionxp = pso.read_u32(playeraddr + 0xe48) - session.startXP
				...
				else -- location ~= session.lastLocation
					if session.lastLocation == _lobby then
						initsessiontime()
						session.startXP = pso.read_u32(playeraddr + 0xe48)
						-- session.lastLocation = location
						-- session.lastFrameTime = now
					end -- if session.lastLocation == _lobby
				end -- if location == session.lastLocation
		...
		if activegamedata.flooritems or activegamedata.inventory then
			...
			local playerindex = pso.read_u32(offset.myPlayerIndex)
		...
		if activegamedata.monsterlist then
			...
			local playerroom1 = pso.read_u16(playeraddr + offset.room)
			...
			for i = 0, entitycount do
				...
				-- thismonster.hp = pso.read_u16(monsteraddr + offset.HP)
				if monsteraddr ~= 0 then
					local monsterid = pso.read_u32(monsteraddr + offset.monsterUnitxtID)
					if monsterid == 45 or monsterid == 73 then
						thismonster.name = readfromunitxt(namegroup, pso.read_u32(monsteraddr + offset.monsterUnitxtID))
						thismonster.hp = pso.read_u32(monsteraddr + offset.bossHP[monsterid])
						thismonster.hpmax = pso.read_u32(monsteraddr + offset.bossMaxHP[monsterid])
						local maxdataptr = pso.read_u32(0x00a43cc8)
						if maxdataptr ~= 0 then -- still has armor layer
							if i == 0 then -- this is the head
								thismonster.ap = pso.read_u32(monsteraddr + offset.maskHP[monsterid])
								thismonster.apmax = pso.read_u32(monsteraddr + 0x20)
							else -- this is a body segment
								thismonster.ap = pso.read_u32(monsteraddr + offset.shellHP[monsterid])
								thismonster.apmax = pso.read_u32(monsteraddr + 0x1c)
							end -- if i == 0
						end -- maxdataptr ~= 0
					else
						-- table.insert(monsteraddrlist, monsteraddr)
						local monsterroom = pso.read_u16(monsteraddr + offset.room)
						-- print(pso.read_u16(monsteraddr + offset.HP))
						-- print(monsterroom .. " == " .. playerroom1 .. " or " .. playerroom2)
						-- print((monsterroom == playerroom1 or monsterroom == playerroom2) and pso.read_u16(monsteraddr + offset.HP) > 0)
						if (monsterroom == playerroom1 or monsterroom == playerroom2) and pso.read_u16(monsteraddr + offset.HP) > 0 then
							-- print("live monster!")
...
local registerGameWindowSize = {
	name = 'registerGameWindowSize',
	description = 'read game window size and register it with CustomHUD',
	run = function()
		local width = pso.read_u16(0x00a46c48)
		if width > 0 then
			local height = pso.read_u16(0x00a46c4a)
			CustomHUD.gameWindowSize = {w = width, h = height}
			return 'complete'
		else
			return 'incomplete'
		end -- if width > 0
...
return {
	name = 'dataSources',
	module = psoData, 
	newTasks = {registerGameWindowSize},
}
